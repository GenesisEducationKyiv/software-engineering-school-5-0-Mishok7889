The code should follow such recommendations.

- Structured logging(use slog instead of simple log.printf)
- Early returns to reduce excessive nesting
- Error wrapping 
- Using options/config/params structs for functions with too many parameters(3 and more)
- Avoid using time.Sleep in tests 
- Using table-driven tests for better readability & maintainability
- Separate unclear or wordy validation logic into separate functions for better readability

We're using the code style in our projects. Scan them and use in evaluation.
